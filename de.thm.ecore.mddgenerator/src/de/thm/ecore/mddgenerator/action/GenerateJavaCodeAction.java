...
public class GenerateJavaCodeAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		GenerateJob job = new GenerateJob("Generating Java code", resEcoreFile, resMymetamodelFile,
				project);
		job.setUser(true);
		job.schedule();
	}
	...
	private class GenerateJob extends Job implements IJobChangeListener {
		...
		long jobBegin;
		Resource resEcoreFile, resMymetamodelFile;
		IProject project;

		public GenerateJob(String name, Resource resEcoreFile, Resource resMymetamodelFile, IProject project) {
			super(name);
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			generator.doGenerate(resEcoreFile, resMymetamodelFile, project, monitor);
